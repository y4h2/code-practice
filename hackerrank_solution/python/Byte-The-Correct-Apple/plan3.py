...
cc_words = set(w for w in open("apple-computers.txt").read().split() if w not in sw_list)
fruit_words = set(w for w in open("apple-fruit.txt").read().split() if w not in sw_list)
